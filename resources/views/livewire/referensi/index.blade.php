<div class="p-5">
    ...
    <div class="flex justify-between mt-5">
        ...
        <a class="btn btn-primary" href="{{ route('referensi.create',['kategori' => $kategori]) }}">Buat</a>
    </div>
            ...
            <div class="overflow-x-auto">
                <table class="table table-xs">
                    ...
                    @foreach($referensi_all as $key => $item)
                        <tr wire:key="{{ $item->id }}">
                            <th>{{ $referensi_all->firstItem() + $key }}</th>
                            <td>{{ $item->nama }}</td>
                            <td>{{ $item->dibuat }}</td>
                            <td>{{ $item->diubah }}</td>
                            <td>
                                <a class="btn btn-xs btn-warning mr-1"
                                   href="{{ route('referensi.edit',['referensi' => $item->id,'kategori' => $kategori]) }}">Edit</a>
